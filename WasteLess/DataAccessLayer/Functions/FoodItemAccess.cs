    ...
    public class FoodItemAccess
    {
        ...
        public List<FoodItem> getFoodList(long id, DateTime leftDate, DateTime rightDate)
        {
            using (var _dcm = new DatabaseConnectionManager())
            {
                ...
                //IEnumerable<FoodItem> food_item_enum = (from _food_item in _dcm.FoodItems where _food_item.User_id == id && _food_item.PurchaseDate <= nowDate && _food_item.PurchaseDate >= nowDate.AddDays(-7) select _food_item).AsEnumerable();
                return food_item_enum.ToList();
                //IEnumerable<FoodItem> account = (from _account in _dcm.BankAccount where _account.User_id == query_user.Id select _account).AsEnumerable();
            }
        }
